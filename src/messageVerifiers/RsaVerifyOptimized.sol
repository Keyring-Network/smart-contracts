...
contract RsaVerifyOptimized is RsaMessagePacking {
        ...
        returns (bool)
    {
        ...
        assembly ("memory-safe") {
            ...
            if iszero(digestAlgoWithParamLen) { result := false }
            ...
            for { let i := add(decipher, 34) } /* 0x20+2 */ lt(i, _maxIndex) { i := add(i, 1) } {
                if lt(byte(0, mload(i)), 0xff) { result := false }
            }
            ...
            // }
            //
            if gt(byte(0, mload(_maxIndex)), 0) { result := false }
        }
        if (!result) {
            return false;
        }
